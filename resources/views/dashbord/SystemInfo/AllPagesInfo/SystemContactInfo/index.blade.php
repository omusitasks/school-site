...
<html lang="en">
  <head>
    <meta charset="utf-8">
      ...
      <title>Dashbord | Contact Information</title>
  ...
  <body class="  ">
                        ...
                        <div>
                            ...
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb ">
                                   ...
                                   <li class="breadcrumb-item active" aria-current="page">Contact Information List</li>
                                </ol>
                             </nav>

                            <h4 class="mb-3">Contact Information List</h4>
                        </div>
                        @if (Auth::user()->hasPermissionTo('Tag create'))
                        <a href="{{ route('dashboard.contact_information.create') }}" class="btn btn-primary add-list"><i class="las la-plus mr-3"></i>Add Contact Information</a>
                        @endif
                    </div>
                </div>
                <div class="col-lg-12">
    ...
    <tbody class="light-body">
        @foreach ($contact_information as $ic)
            <tr>
                <td>{{ $loop->iteration }}</td>
                <td>{{ $ic->company_phone_number }}</td>
                <td>{{ $ic->phone_icon_id }}</td>
                <td>
                    <div class="d-flex align-items-center list-action">
                        @auth
                            @if (Auth::user()->hasRole('admin'))
                                <a href="{{ route('dashboard.contact_information.show', $ic->id) }}" class="badge badge-info mr-2" data-toggle="tooltip" data-placement="top" title="View"><i class="ri-eye-line mr-0"></i></a>
                                <a href="{{ route('dashboard.contact_information.edit', $ic->id) }}" class="badge bg-success mr-2" data-toggle="tooltip" data-placement="top" title="Edit"><i class="ri-pencil-line mr-0"></i></a>
                                

                                <form id="deleteCourseForm{{ $ic->id }}" action="{{ route('dashboard.email_information.destroy', $ic->id) }}" method="POST">
                                    @csrf
                                    @method("DELETE")
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                Are you sure you want to delete {{ $ic->company_phone_number }}?
                                            </div>
